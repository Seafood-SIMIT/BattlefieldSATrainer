...
def main():
    ...
    # Call baks
    log_dir = Path("training_logs") / str(args.model)
    _ensure_logging_dir(log_dir)
    logger = pl.loggers.TensorBoardLogger(log_dir)
    experiment_dir = logger.log_dir

    goldstar_metric = "validation/cer" if hp.kg.loss in ("transformer",) else "validation/loss"
    filename_format = str(args.model) + "epoch={epoch:04d}-validation.loss={validation/loss:.3f}"
    ...
    if best_model_path:
        ...
        trainer.test(kg_litmodel, datamodule=data)
